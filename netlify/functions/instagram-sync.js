...
const LIST_FIELDS = process.env.LIST_FIELDS === '1';
...
async function fetchWebflowCollectionFields() {
  if (!WEBFLOW_COLLECTION_ID || !WEBFLOW_TOKEN) throw new Error('Missing Webflow env vars');
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to fetch collection: ${text}`);
  }
  const json = await res.json();
  console.log('Raw collection payload:', JSON.stringify(json, null, 2));
  // Return simplified list of fields with apiId, name and type
  return (json.fields || []).map(f => ({ apiId: f.apiId || f.slug || f.id, name: f.name || f.displayName, type: f.type || f.fieldType }));
}

exports.handler = async function () {
  try {
    if (LIST_FIELDS) {
      const fields = await fetchWebflowCollectionFields();
      console.log('Collection fields (apiId | name | type):');
      console.log(fields);
      return { statusCode: 200, body: 'Listed collection fields. Check logs.' };
    }
    ...
    if (SKIP_WEBFLOW) {
      ...
      return { statusCode: 200, body: 'Dry run complete' };
    }
